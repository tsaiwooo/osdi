...
void sysinfo()
{
    ...
    get_info_mem(MBOX_ARM_MEMORY, 8);
    if (mbox[5] == 0) {
...
void get_info(unsigned int tags, int size)
{
    ...
    mailbox_call(
        MBOX_CH_PROP); // message passing procedure call, you should implement it
                       // following the 6 steps provided above.
}

void get_info_mem(unsigned int tags, int size)
{
...
{
    uart_printf(""); // if lose this, it will call exception, but i dont know
    // why?
...
int sys_mailbox(unsigned char ch, unsigned int* sys_mbox)
{
    uart_printf(""); // if lose this, it will call exception, but i dont know
    // why?
    ...
    *MBOX_WRITE = r;
    ...
    while (1) {
        ...
        if (r == *MBOX_READ) {
            /* is it a valid successful response? */
            return sys_mbox[1] == MBOX_RESPONSE;
        }
    }
    return 0;
}