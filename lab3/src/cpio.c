                ...
                unsigned long *filesize_, char **data,cpio_newc_header **next)
{
    ...
    *data = (char *)archive + offset;
    // *data = (char *)align_up(((unsigned long *)archive) + sizeof(struct cpio_newc_header) + 
...
void ls()
{
    ...
    while(header != 0){
        int finish = cpio_parse_header(header,&filename,&filesize,&data,&header);
        if(finish) break;
