...
typedef struct{
    ...
    uint32_t totalsize; // contain the total size in bytes of the devicetree data structure.
    uint32_t off_dt_struct; // offset in bytes of the structure block (see section 5.4) from the beginning of the header.  
    uint32_t off_dt_strings; // contain the offset in bytes of the strings block (see section 5.5) from the beginning of the header.
...
// It shall be followed by extra data describing the property. This data consists first of the property’s length
